...
This project deploy ML Flask App into Azure Clould as Web Application. The insctruction consits all steps of deployments and demonstration 
## Project Plan
...
In the end, arcitecture of deployed project will follow the diagramm below:
...
3. Instead of autogenerated code for `main.yml` insert the code from the same file in `.github/workflows`
4. Commit and check current progress of workflows
![Alt text](/screenshots/scrWorkflow.png?raw=true "Title")
5. Clicking on specific workflow, you can find more information about it. make sure, that all workflows were completed succsesfully.

### Clone repository into Azure Cloud Shell 
1. Create (if you don't have one) Azure Accoount
...
8. Use public adress for deployed web app and check using public adress
...
5. Choose Python to Linux Web App on Azure and web app, which was deployed previosly in Azure Cloud Shell
...
8. Ensure, that deployment completed sucsesfully
![Alt text](/screenshots/AllDonePipeline.png?raw=true "Title")
9. Push commint, which changed in `app.py` homepage
![Alt text](/screenshots/CommitChange.png?raw=true "Title")
10. Ensure that push trigger CD process
![Alt text](/screenshots/commitTriggerBuild.png?raw=true "Title")
11. Check change which was delivered by pipiline
...
## Enhancements
This project demonstrates basic functionality and provide only a simple insights of CI/CD pipiline. There is a huge field for improvment. First of all, it's great to have a good test coverage and test code with each commit using GitHub Action or during job buiding in CD. Other approache of upgrading project is develop user friendly interface for project.
